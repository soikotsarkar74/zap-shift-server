...
//         app.get('/parcels/:id', async (req, res) => {
//             const result = await parcelsCollection.findOne({ _id: new ObjectId(req.params.id) });
//             res.send(result);
//         });
...
//         app.patch('/parcels/:id/update', async (req, res) => {
//             try {
//                 const { parcelName, parcelType, parcelWeight, receiverName, receiverPhone, receiverRegion, receiverDistrict, receiverAddress } = req.body;
//                 const result = await parcelsCollection.updateOne(
//                     { _id: new ObjectId(req.params.id) },
//                     {
//                         $set: {
...
//         // Tracking
//         app.get('/trackings/:trackingId/logs', async (req, res) => {
//             const result = await trackingsCollection.find({ trackingId: req.params.trackingId }).toArray();
//             res.send(result);
//         });

//         app.get('/riders/dashboard-stats', verifyFBToken, async (req, res) => {
//     try {
//         const email = req.query.email;
...
// MongoDB Connection - ভেরসেলের জন্য অপটিমাইজড
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.6lmq5wd.mongodb.net/?appName=Cluster0`;
const client = new MongoClient(uri, {
    serverApi: {
        version: ServerApiVersion.v1,
...
// Database functions - ভেরসেলের জন্য আলাদা ফাংশন
let db, userCollection, parcelsCollection, paymentCollection, ridersCollection, trackingsCollection;
...
async function connectDB() {
    ...
    return { db, userCollection, parcelsCollection, paymentCollection, ridersCollection, trackingsCollection };
}
...
// Tracking Log
const logTracking = async (trackingId, status) => {
    const { trackingsCollection } = await connectDB();
    const log = {
        trackingId,
        status,
        details: status.split('_').join(' '),
...
app.patch('/parcels/:id', async (req, res) => {
    const { parcelsCollection, ridersCollection } = await connectDB();
    const { riderId, riderName, riderEmail, trackingId } = req.body;
    const result = await parcelsCollection.updateOne(
        { _id: new ObjectId(req.params.id) },
        {
...
app.patch('/payment-success', async (req, res) => {
    const { parcelsCollection, paymentCollection } = await connectDB();
    const sessionId = req.query.session_id;
    const session = await stripe.checkout.sessions.retrieve(sessionId);
    const transactionId = session.payment_intent;
...
// Riders
app.get('/riders', async (req, res) => {
    const { ridersCollection } = await connectDB();
    const query = {};
    const { status, district, workStatus } = req.query;
    if (status) query.status = status;
    if (district) query.district = district;
...
app.patch('/riders/:id', verifyFBToken, verifyAdmin, async (req, res) => {
    const { ridersCollection, userCollection } = await connectDB();
    const { status, email } = req.body;
...
app.get('/riders/dashboard-stats', verifyFBToken, async (req, res) => {
    try {
        const { ridersCollection, parcelsCollection } = await connectDB();
        const email = req.query.email;
